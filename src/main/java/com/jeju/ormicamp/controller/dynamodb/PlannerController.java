...
public class PlannerController {
    ...
    // POST /api/planner
    @PostMapping
    public ResponseEntity<BaseResponse<String>> savePlanner(@RequestBody PlannerReqDto reqDto) {
        log.info("플래너 저장 요청: SessionId={}, Date={}", reqDto.getSessionId(), reqDto.getDate());
        plannerService.updatePlanner(reqDto);
        return ResponseEntity.ok(
                BaseResponse.success("플래너가 성공적으로 저장되었습니다.", null)
        );
    }
    ...
    public ResponseEntity<BaseResponse<PlannerResDto>> getPlanner(
        ...
        return ResponseEntity.ok(
                BaseResponse.success("플래너가 조회되었습니다.", resDto)
        );
    }
    ...
    public ResponseEntity<BaseResponse<List<PlannerResDto>>> getAllPlanners(@PathVariable String sessionId) {
        log.info("전체 일정 조회: SessionId={}", sessionId);
        List<PlannerResDto> list = plannerService.getAllPlanners(sessionId);
        return ResponseEntity.ok(
                BaseResponse.success("일정이 조회되었습니다.", null)
        );
    }
    ...
    public ResponseEntity<BaseResponse<String>> deletePlanner(
            @PathVariable String sessionId,
            @PathVariable String date) {
        log.info("플래너 삭제: SessionId={}, Date={}", sessionId, date);
        plannerService.deletePlanner(sessionId, date);
        return ResponseEntity.ok(
                BaseResponse.success("일정이 삭제되었습니다.", null)
        );
    }
}